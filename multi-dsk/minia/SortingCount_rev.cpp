...
void sorting_count(Bank *Sequences, char *prefix, int max_memory, int max_disk_space, bool write_count, int verbose)
{
    ...
    uint64_t volume = Sequences->estimate_kmers_volume(smallestKmer,(sizeof(kmer_type)+sizeof(int))*8) * 2;  //Since there are totalKmers no of kmers and an upper bound can be estimated by using the smallest size of kmer. Added by Raunaq, the factor of two is for doubling the reads to forward and reverse strandds 
    
	uint32_t nb_passes = ( volume / max_disk_space ) + 1;
	//printf("number of passes 1 %lu\n",nb_passes);
    int passes_hash ;
    ...
    int partitions_hash;
	uint64_t volume_without_length =  (volume*1.0*sizeof(kmer_type))/(sizeof(kmer_type)+sizeof(int));
	//printf("Volume without length %llu \n",volume_without_length);
    ...
    do
    {
		...
		volume_per_pass = (volume_per_pass*1.0*sizeof(kmer_type) ) / (sizeof(kmer_type)+sizeof(int));
        nb_partitions = ( volume_per_pass * 2 / max_memory ) + 1; // two needed as only two hashes will be used
    ...
    while (1);
	//printf("volume per pass %llu and no of partitions %lu \n",volume_per_pass,nb_partitions);
	...
	//printf("number of partitions %lu volume per partition %llu \n",nb_partitions, volume_per_partition);
    //int size_for_reestimation = ceil((passes_hash + partitions_hash)*1.8);
	int size_for_reestimation = ceil((passes_hash + partitions_hash));
	...
	//printf("number of passes 3 %lu\n",nb_passes);
	//printf("Total volume %llu volume per partition %llu\n",volume, volume_per_partition);
