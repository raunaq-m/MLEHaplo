...
long seed_lmer(char *seq,int size)
{
	long x;
	int i;
	x=0;
...
kmer_type  codeSeed(char *seq, int size, kmer_type kmerMask)
{
    int i;
    kmer_type x;
...
// return the k nucleotides from the right of the kmer
uint32_t code2first_n_nucleotide(kmer_type code, int n ) 
{
	return code2first_n_nucleotide(code,n,0);
}

uint32_t code2first_n_nucleotide(kmer_type code, int n, int start) 
{
	kmer_type temp = code;
	int result = 0;
	int result1;
...
//	for(int j=0;j<n;j++)
//        {
//                result = result*4 + first_nucleotide(temp&3);
//                temp = temp>>2;
//        }
   	kmer_type pass_temp = temp & ( (((kmer_type)1)<<(2*n))-1);
#ifdef _largeint
    LargeInt<KMER_PRECISION> t = pass_temp;
    result1 = t.toInt();
#else
#ifdef _ttmath
    ttmath::UInt<KMER_PRECISION> t = pass_temp;
    t.ToInt(result1);
#else
    result1 = pass_temp;
#endif
...
kmer_type codeSeed_bin(char *seq) {
	return codeSeed_bin(seq,sizeKmer);
}

kmer_type  codeSeed_bin(char *seq, int size)
{
    int i;
    kmer_type x;
