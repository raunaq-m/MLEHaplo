...
#include "loadKmers.h" //Added by Rauaq on 19/09/2013
...
class BinaryBankConcurrent :  public BinaryBank
{
    ...
    void flush(int tid);
    int find_error(); // Added by Raunaq for error check
    void close();
...
class KmersBuffer
{
    ...
    unsigned int  read_write_buffer_size;
     int cpt_binSeq_read;
     int binSeq_toread;
     int cpt_read_count; //Added by Raunaq
     int smallK;	 //Added by Raunaq
